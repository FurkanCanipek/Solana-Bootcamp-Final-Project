...
async function main(feePayer: Keypair) {
    ...
    /**
     * Derive the Tree Metadata so we can retrieve it later
     */
    const [TreePub] = deriveTreeMetadataPDA(
        {
            mint: mint.publicKey,
        },
        progId,
    );
    console.info("+==== Tree Metadata Address ====+");
    console.info(TreePub.toBase58());
    ...
    await advanceAgeSendAndConfirm({
        treeSeedMint: TreePub,
        signers: {
            feePayer: feePayer
        }
    })
    Tree = await getTreeMetadata(TreePub);
    console.info("+====Advanced age Tree Metadata ====+");
    console.info(Tree);
